...
class HTTPParser:
    ...
    def parse(self, data: bytearray, eof: bool = False) -> Tuple[Optional[HTTPMessage], int]:
        ...
        # For requests, assume no body if neither CL nor chunked (GET, HEAD, DELETE, …)
        if not is_response:
            if get_content_length() is None and not has_chunked():
                msg = HTTPMessage(
                    start_line=start_line,
                    headers=headers,
                    body=b"",
                ...
                return msg, body_start
            # Fall through to the Content-Length / chunked logic below
            # for requests that do carry a body (POST, PUT, PATCH, …)
